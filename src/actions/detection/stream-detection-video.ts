...
export const streamDetectionVideo = (
  videoId: string,
  callbacks: StreamCallbacks
) => {
  if (!videoId) {
    callbacks.onError?.('Detection: No video ID available')
    throw new Error('Detection: No video ID available')
  }

  const wsUrl = ENDPOINTS.DETECTIONS.STREAM_DETECTIONS(videoId)
